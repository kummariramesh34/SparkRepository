...
basedir = '/home/team124/hackathon/SparkRepository/network-packet-analysis/trainingdata/'
...
filename = '/home/team124/hackathon/SparkRepository/network-packet-analysis/model/sqli_model.sav'
...
def Check_is_sql(sql):
    ...
    return_val = "VALID-REQUEST"
    if res == 1:
        return_val = "SQL-INJECTION ATTACK"
    else:
        return_val = "VALID-REQUEST"
    return return_val

# read data from file. You should change this path!!
tcpdumpdir = '/home/team124/hackathon/SparkRepository/network-packet-analysis/data/tcp_dump'
tcpdumpfilelist = os.listdir(tcpdumpdir)
tcp_df_list = []
for file in tcpdumpfilelist:
    if file == '.DS_Store':
        continue
    tcpdf = pd.read_csv(os.path.join(tcpdumpdir,file), sep='Aw3s0meSc0t7', names=['raw_sql'], header=None, engine='python')
    tcp_df_list.append(tcpdf)

# god pandas make to us a dataframe like excel format
tcpdataframe = pd.concat(tcp_df_list, ignore_index=True)
tcpdataframe.dropna(inplace=True)

tcpdataframe['predict_val'] = tcpdataframe['raw_sql'].map(lambda x: Check_is_sql(x))
print(tcpdataframe)
ts = time.time()
st = datetime.datetime.fromtimestamp(ts).strftime('%Y_%m_%d_%H_%M_%S')
resultsfilename = '/home/team124/hackathon/SparkRepository/network-packet-analysis/data/results/predictions_'+st
tcpdataframe.to_csv(resultsfilename)
print('Results successfully written in results dir :: Result File path'+resultsfilename)
